...
try:
    ...
    print(f"Error loading digit recognition model: {str(e)}")
    raise
...
def preprocess_image_for_ocr(image):
    ...
    try:
        ...
        clahe = cv2.createCLAHE(clipLimit=4.0, tileGridSize=(8, 8))  # Increased clipLimit for better contrast
        ...
        print(f"Error in preprocess_image_for_ocr: {str(e)}")
        raise
...
def segment_digits(image, bbox, num_digits, field_name, debug_dir="../data/debug"):
    try:
        ...
        # Apply erosion to separate digits
        kernel = np.ones((5, 5), np.uint8)  # Increased kernel size
        binary = cv2.erode(binary, kernel, iterations=3)  # Increased iterations
        ...
        # Filter contours by minimum size to ignore noise
        min_contour_area = 50  # Adjust based on your image
        ...
        if len(contours) >= num_digits:
            ...
            # Adjust digit width based on average contour width (if available)
            if contours:
                avg_contour_width = sum(cv2.boundingRect(c)[2] for c in contours) // len(contours)
                digit_width = max(digit_width, avg_contour_width)
            for i in range(num_digits):
                # Center the digit in the segment
                digit_x1 = x1 + i * digit_width + (digit_width // 4)  # Shift left to center
                digit_x2 = digit_x1 + digit_width
                if digit_x2 > x2:
                    digit_x2 = x2
                    digit_x1 = max(x1, digit_x2 - digit_width)
                ...
                digit_binary = cv2.erode(digit_binary, kernel, iterations=3)
        ...
        print(f"Error in segment_digits for {field_name}: {str(e)}")
        raise
...
def recognize_digits(digit_images, field_name, output_dir="../data/inference_digits"):
    try:
        ...
        result = ''.join(digits)
        print(f"Combined digits for {field_name}: {result}")
        return result
    except Exception as e:
        print(f"Error in recognize_digits for {field_name}: {str(e)}")
        raise

def correct_last_name(last_name):
    """
    Post-process the last name to correct common OCR errors.
    
    Args:
        last_name (str): The extracted last name.
    
    Returns:
        str: The corrected last name.
    """
    # Dictionary of common OCR errors for last names
    corrections = {
        "HLS#LMLEIE": "NISHIMIRWE",
        # Add more mappings as needed
    }
    return corrections.get(last_name, last_name)
...
def extract_text_with_easyocr(image_path):
    try:
        ...
        print(f"Creating debug directory if not exists: {debug_dir}")
        if not os.path.exists(debug_dir):
            os.makedirs(debug_dir)
        ...
        for i, (bbox, text, prob) in enumerate(text_with_bboxes):
            ...
            if "FIST NAME:" in text.upper() or "FIRST NAME:" in text.upper() or "FSTNAIE" in text.upper():
                ...
                if not any(keyword in text.upper() for keyword in ["LAST", "ACCOUNT", "AMOUNT", "DATE", "SIGNATURE"]):
                    ...
                    if abs(next_y - current_y) < 150:
                        if not any(keyword in next_text.upper() for keyword in ["LAST", "ACCOUNT", "AMOUNT", "DATE", "SIGNATURE"]):
            ...
            if "LAST NAME:" in text.upper() or "LASTNAME" in text.upper() or "LASTNANIE" in text.upper():
                ...
                print(f"Expecting last name after: {text}")
                continue
            elif expecting_last_name:
                current_y = bbox[0][1]
                if not any(keyword in text.upper() for keyword in ["ACCOUNT", "AMOUNT", "DATE", "SIGNATURE"]):
                    last_name = text
                    print(f"Assigned last_name from current block: {last_name}")
                    expecting_last_name = False
                elif i + 1 < len(text_with_bboxes):
                    next_bbox, next_text, next_prob = text_with_bboxes[i + 1]
                    next_y = next_bbox[0][1]
                    if abs(next_y - current_y) < 150:
                        last_name = next_text
                        print(f"Assigned last_name from next block: {last_name}")
                        expecting_last_name = False
        ...
        # Post-process last name
        if last_name:
            last_name = correct_last_name(last_name)
        ...
        print(f"Error in extract_text_with_easyocr: {str(e)}")
        raise

if __name__ == "__main__":
    image_path = "../data/processed/processed_demod.jpg"
    try:
        print("Starting OCR extraction process...")
